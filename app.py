...
SUPABASE_BUCKET_VIDEOS = "RealRoll"
...
class Video(db.Model):
    ...
    blob_url = db.Column(db.String(500), nullable=False)  # we keep name same for backward compatibility
...
def upload_file_to_supabase(bucket_name, file_obj, filename):
    """Upload file to Supabase storage and return public URL"""
    unique_name = f"{uuid4()}_{secure_filename(filename)}"
    
    # Upload file (raises exception if it fails)
    supabase.storage.from_(bucket_name).upload(unique_name, file_obj)
    ...
    try:
        if request.method == 'POST':
            ...
            try:
                ...
                # Upload to Supabase
                blob_url = upload_file_to_supabase(SUPABASE_BUCKET_VIDEOS, file_bytes, file.filename)
...
def edit_profile():
    ...
    try:
        ...
        if avatar_file and avatar_file.filename:
            avatar_url = upload_file_to_supabase(SUPABASE_BUCKET_AVATARS, avatar_file, avatar_file.filename)
...
def delete_video():
    ...
    try:
        ...
        return {'success': False, 'error': 'Invalid video id'}, 400
    ...
    try:
        delete_file_from_supabase(SUPABASE_BUCKET_VIDEOS, video.blob_url)
        db.session.delete(video)
        db.session.commit()
        # Respond based on client expectations
...
def dashboard():
    ...
    try:
        ...
        for video in videos:
            ...
            # Append data including age_rating
            video_data.append({
                "video": video,
                "comments": comments,
                ...
                "age_rating": video.age_rating  # 👈 added here
            })
